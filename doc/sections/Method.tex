...
This is processing the static data, which is the same computation graph during placement of each node. 
After placing all nodes in a rollout, a different graph can be fed into the GNN to train the RL model on a collection of computation graphs. 
...
Two layers of Graph Convolution \cite{wu2019simplifying} and a graph average pool layer produces a vector embedding.
An attention module is applied to the embedding to highlight relevant components to the current node to be placed. 
The Global Graph Attention Module is the GNN and attention module combination.
...
The embeddings from dynamic data and static data are concatenated and fed to an MLP model with 3 FC layers to create actions. 
...
We see the addition of GGA module provides improved reward and sample efficiency for various computation graphs and different device configuration as shown in section \ref{sec:GGA_result}.
